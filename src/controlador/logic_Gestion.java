...
public class logic_Gestion implements ActionListener, KeyListener, ListSelectionListener, Dimensiones {
    ...
    /**
     * Constructor de la clase logic_Gestion.
     * 
     * @param lb La instancia de la vista Gestion.
     */
    ...
    /**
     * Configura los listeners para los componentes de la GUI.
     */
    ...
    /**
     * Configura el objeto de acceso a datos.
     * 
     * @param datos La instancia de DataAccess.
     */
    ...
    /**
     * Configura el tipo de ventana a gestionar y carga los datos correspondientes.
     * 
    ...
    /**
     * Restablece los componentes de la interfaz de usuario a su estado predeterminado.
     */
    private void reset() {
    ...
    /**
     * Carga los datos en la vista de Gestión según el tipo actual.
     */
    ...
    /**
     * Actualiza los datos de la lista en la vista según el tipo actual.
     */
    ...
    /**
     * Actualiza la información mostrada en el área de texto de información según el
    ...
    public void actualizarInformacion() {
        ...
        switch (tipo) {
            case CLIENTE:
                ...
                lb.txt_informacion.setText(String.format(
                    "Nombre: %s\nDireccion: %s\nCedula: %s\nTelefono: %s\nCorreo: %s\nTipo: %s", 
                    tm_usuario.getNombre(),
                    tm_usuario.getDireccion(), 
                    tm_usuario.getCedula(), 
                    tm_usuario.getTelefono(), 
                    tm_usuario.getCorreo(), 
                    tm_usuario instanceof Administrador ? "Administrador" : 
                        tm_usuario instanceof Empleado ? "Empleado" : "Cliente"
                ));
                break;
            case PROVEEDOR:
                ...
                lb.txt_informacion.setText(String.format(
                    "Nombre: %s\nCorreo: %s\nCedula: %s\nCodigo: %s\nTelefono: %s\nRazon Social: %s",
                    tm_proveedor.getNombreCompleto(), 
                    tm_proveedor.getEmail(), 
                    tm_proveedor.getDni(),
                    tm_proveedor.getCodigo(), 
                    tm_proveedor.getTelefono(), 
                    tm_proveedor.getRazonSocial()
                ));
                break;
            case PRODUCTO:
                ...
                lb.txt_informacion.setText(String.format(
                    ...
                    tm_producto.getname(), 
                    tm_producto.getDesc(), 
                    tm_producto.getCodigo(), 
                    tm_producto.getprecio(),
                    tm_producto.getstock(), 
                    tm_producto.getproveedor().getNombreCompleto()
                ));
                break;
            default:
                break;
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (e.getSource() == lb.lst_resultados) {
            if (lb.lst_resultados.getSelectedValue() != null) {
                switch (tipo) {
                    case CLIENTE:
                        tm_usuario = tm_datos.getClientes()
                            .get(tm_datos.getClientes().indexOf(lb.lst_resultados.getSelectedValue()));
                        break;
                    case PROVEEDOR:
                        tm_proveedor = tm_datos.getProveedores()
                            .get(tm_datos.getProveedores().indexOf(lb.lst_resultados.getSelectedValue()));
                        break;
                    case PRODUCTO:
                        tm_producto = tm_datos.getProductos()
                            .get(tm_datos.getProductos().indexOf(lb.lst_resultados.getSelectedValue()));
                        break;
                    default:
                        break;
                }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == lb.btn_buscar) {
            switch (tipo) {
                case CLIENTE:
                    lb.lst_resultados.setListData(
                        tm_datos.getClientes().stream()
                ...
                case PROVEEDOR:
                    lb.lst_resultados.setListData(
                        tm_datos.getProveedores().stream()
                        .filter(p -> p.getNombreCompleto().toLowerCase().contains(lb.txt_busqueda.getText().toLowerCase()))
                        .toArray()
                    );
                    break;
                case PRODUCTO:
                    lb.lst_resultados.setListData(
                        tm_datos.getProductos().stream()
            ...
            guardado = false;
            if (JOptionPane.showConfirmDialog(lb, "¿Desea eliminar el elemento seleccionado?", "Confirmar Eliminación", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                switch (tipo) {
                    case CLIENTE:
            ...
            guardar.start();
            guardado = true;
            JOptionPane.showMessageDialog(lb, "Guardado con éxito", "Guardado", JOptionPane.INFORMATION_MESSAGE);
